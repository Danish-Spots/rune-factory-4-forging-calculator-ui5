...
const MaterialTableDelegate = Object.assign({}, MaterialBaseDelegate, TableDelegate);
...
MaterialTableDelegate.addItem = async (table: Table, propertyKey: string) => {
	const propertyInfo = MaterialTablePropertyInfo.find((p) => p.key === propertyKey) as TablePropertyInfo;
	return _createColumn(propertyInfo, table);
};

MaterialTableDelegate.updateBindingInfo = (table, bindingInfo) => {
	TableDelegate.updateBindingInfo.call(MaterialTableDelegate, table, bindingInfo);
