...
var MaterialFilterBarDelegate = Object.assign({}, MaterialBaseDelegate, FilterBarDelegate);
...
const _createValueHelp = async (filterBar: FilterBar, propKey: string) => {
	...
	const valueHelp = (await Fragment.load({
		name: `${path}${(filterBar.getPayload() as FilterBarPayload).valueHelp[propKey]}`,
	})) as unknown as ValueHelp;
	filterBar.addDependent(valueHelp);
	return valueHelp;
};

const _createFilterField = async (id: string, property: PropertyInfo, filterBar: FilterBar) => {
	...
	if ((filterBar.getPayload() as FilterBarPayload).valueHelp[propKey]) {
		...
		let valueHelp = dependents.find((dependent) => dependent.getId().includes(propKey)) as ValueHelp;
...
MaterialFilterBarDelegate.fetchProperties = async () => MaterialTablePropertyInfo;

MaterialFilterBarDelegate.addItem = async (filterBar: FilterBar, propertyKey: string) => {
	const prop = MaterialTablePropertyInfo.find((p) => p.key === propertyKey) as PropertyInfo;
